...
if (!function_exists('inwave_social_sharing')) {
    ...
    function inwave_social_sharing($link, $text, $title, $tag = '', $return = false)
    {
        ...
        if ($inwave_theme_option['sharing_facebook']) {
            $shareLink = 'https://www.facebook.com/sharer/sharer.php?s=100&amp;p[title]=' . $title . '&amp;p[url]=' . $link . '&amp;p[summary]=' . $text .'&amp;u='. $link;;
            $html .= ($tag ? '<' . $tag . '>' : '') . '<a class="share-buttons-fb" target="_blank" href="' . esc_url($shareLink) . '" title="' . esc_attr_x('Share on Facebook','title','motohero') . '" onclick="return iwOpenWindow(\'' . esc_js($shareLink) . '\')"><i class="fa fa-facebook"></i></a>' . ($tag ? '</' . $tag . '>' : '');
        }
